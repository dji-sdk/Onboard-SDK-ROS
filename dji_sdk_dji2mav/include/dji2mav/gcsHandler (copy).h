/*****************************************************************************
 * @Brief     Handle Ground Control Stations. ROS-free and mav-free singleton
 * @Version   1.1
 * @Author    Chris Liu
 * @Created   2015/10/31
 * @Modified  2015/11/15
...
#ifndef _DJI2MAV_GCSHANDLER_H_
#define _DJI2MAV_GCSHANDLER_H_
...
namespace dji2mav {
    ...
    class GcsHandler {
        public:
             ...
             * @return  The only instance of GcsHandler
             * @warning UNSAFE FOR MULTI-THREAD! Should be called BEFORE fork
             */
            static GcsHandler* getInstance() {
                if(NULL == m_instance) {
                    try {
                        m_instance = new GcsHandler();
                    } catch(std::bad_alloc &m) {
                        std::cerr << "New instance of GcsHandler fail: " 
